...
const baseUrl = "https://xv-victoria.vercel.app";
...
async function generarListaInvitados(filtro = "") {
  ...
  const querySnapshot = await db
    .collection("invitadosXVVictoriaOctubre18")
...
window.agregarInvitado = async function () {
  ...
  await db.collection("invitadosXVVictoriaOctubre18").add({
...
window.eliminarInvitado = async function (id) {
  ...
  if (confirmacion) {
    mostrarCargando(true);
    await db.collection("invitadosXVVictoriaOctubre18").doc(id).delete();
